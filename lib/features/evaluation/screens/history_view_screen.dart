...
class _HistoryViewScreenState extends State<HistoryViewScreen> {
  ...
  Future<void> _delete(String id) async {
    await EvaluationStorage.deleteResult(id);
    _load();
  }
  ...
  Future<void> _clearAll() async {
    ...
    if (confirm == true) {
      await EvaluationStorage.clearAll();
      _load();
    }
  }

  Future<void> _shareSingle(EvaluationResult result) async {
    final buffer = StringBuffer();
    buffer.writeln('📖 نتيجة تقييم تلاوة القرآن');
    buffer.writeln('━━━━━━━━━━━━━━');
    buffer.writeln('الطالب: ${result.studentName}');
    buffer.writeln('المرحلة: ${result.stage}');
    buffer.writeln('رقم الصفحة: ${result.pageNumber}');
  ...
  Future<void> _shareAll() async {
    final buffer = StringBuffer();
    buffer.writeln('📊 سجل نتائج تقييم التلاوة');
    buffer.writeln('━━━━━━━━━━━━━━━━━━');

    for (int i = 0; i < _results.length; i++) {
      final r = _results[i];
      buffer.writeln(
        '${i + 1}. ${r.studentName} | ${r.stage} | صفحة ${r.pageNumber} | ${r.grade} | '
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFFF1F5F9),
        elevation: 0,
        title: const Text('سجل النتائج', style: TextStyle(color: Colors.black87)),
        leading: IconButton(
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  Text('لا توجد نتائج محفوظة',
                      style: TextStyle(color: Colors.grey.shade500, fontWeight: FontWeight.w500)),
                  const SizedBox(height: 4),
                  Text('قم بتقييم طالب أولاً',
